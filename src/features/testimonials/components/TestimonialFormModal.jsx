// src/features/testimonials/components/TestimonialFormModal.jsx
import { useState, useEffect, useRef } from 'react';
import Modal from '../../../components/ui/Modal';
import Button from '../../../components/ui/Button';
import { compressAndUploadImage } from '../../../services/imgbb/imgbbService';

const EMPTY_FORM = { name: '', role: '', content: '', photo: '', order: '' };

export default function TestimonialFormModal({ isOpen, onClose, onSubmit, initialData, isSaving }) {
  ...
  async function handleFileChange(e) {
    ...
    try {
      ...
      console.error('[TestimonialFormModal] Error procesando imagen:', err);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isEditMode ? 'Editar Testimonio' : 'Agregar Testimonio'}
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* ── Rol / relación con la ONG ──────────────────────────────── */}
        <Field label="Rol o relación con Munay Perú" required>
          <input
            type="text"
            required
            value={form.role}
            onChange={(e) => handleChange('role', e.target.value)}
            placeholder="Ej. Voluntaria desde 2023"
            className="input-base"
          />
        </Field>

        {/* ── Contenido del testimonio ───────────────────────────────── */}
        <Field label="Testimonio" required>
          <textarea
            required
            rows={4}
            value={form.content}
